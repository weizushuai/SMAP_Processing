...
#create for loop to process each SMAP file
#create variable for names in writeRaster
#create count variable
rasterNames <- smapFiles[,1]
count <- 1
for (i in dateSeq) {
  #download SMAP data in hdf5 format
  print("DOWNLOADING SMAP FILES...")
  downloads <- download_smap(smapFiles[count,], directory = dlDir, overwrite = FALSE)
  ...
  outName <- paste0(rasterNames[count], "_SOIL_MOISTURE")
  writeRaster(smRaster, filename = paste(outPath, outName), format = "GTiff", overwrite = TRUE)
  
  count <- count + 1
}

print("ALL CODE FINISHED!!!")